...
object CqlParser extends JavaTokenParsers with Helpers {
  ...
  def selectStatement: Parser[SelectStatement] = {
    ...
    // TODO
    val SelectStatementWithDefaults = SelectStatement(_: Boolean, _: SelectStatement.SelectClause, _: TableName, None, None, None, false)
    "SELECT".i ~> json ~ select ~ ("FROM" ~> tableName) ^^^^ SelectStatementWithDefaults
  }
